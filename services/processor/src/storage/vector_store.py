...
# from services.processor.src.models.document import DocumentChunk
...
class VectorStore:
    ...
    def _initialize_collection(self):
        """Create necessary collection if it doesn't exist."""
        collection_name = "document_chunks"
        
        try:
            # Check if collection exists
            response = requests.get(f"{self.base_url}/api/v2/collections")
            ...
            if not collection_exists:
                ...
                response = requests.post(f"{self.base_url}/api/v2/collections", json=create_data)
                response.raise_for_status()
                print(f"Created vector store collection: {collection_name}")
            else:
                print(f"Vector store collection already exists: {collection_name}")
    ...
    def store_chunk(self, chunk: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Convert tag_context to string if it's a list
            if isinstance(tag_context, list):
                tag_context = ", ".join(tag_context)
            ...
            # For a simple test, we'll use a random vector (in production, use a real embedding model)
            import random
            ...
            response = requests.post(
                f"{self.base_url}/api/v2/collections/document_chunks/add", 
                json=add_data
            )
            response.raise_for_status()
    ...
    def store_chunks(self, chunks: List[Dict[str, Any]]) -> bool:
        """Store multiple document chunks in vector database."""
        try:
            # For a simple test, we'll use random vectors
            import random
    ...
    def search(self, query: str, top_k: int = 5, filter_dict: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/v2/collections/document_chunks/query", 
                json=query_data
            )
            response.raise_for_status()
    ...
    def delete_document_chunks(self, doc_id: str) -> bool:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/v2/collections/document_chunks/delete", 
                json=delete_data
            )
            response.raise_for_status()
