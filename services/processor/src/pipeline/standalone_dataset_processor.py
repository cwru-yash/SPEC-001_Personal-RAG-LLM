...
#!/usr/bin/env python3
# enhanced_standalone_processor.py - Enhanced version with spreadsheet support
...
class ProcessedDocument:
    ...
    category: str
    doc_type: str  # 'content' or 'metadata'
    content_format: str  # 'pdf', 'xlsx', 'csv', etc.
    text_content: str
    metadata: Dict[str, Any]
    file_size: int
    processing_time: float
...
class EnhancedTextExtractor:
    """Enhanced text extractor supporting multiple file types."""
    ...
    def extract_text(file_path: str) -> tuple[str, str, List[str]]:
        ...
        if file_ext == '.pdf':
            return EnhancedTextExtractor._extract_pdf(file_path)
        elif file_ext in ['.xlsx', '.xls']:
            return EnhancedTextExtractor._extract_excel(file_path)
        elif file_ext == '.csv':
            return EnhancedTextExtractor._extract_csv(file_path)
        elif file_ext in ['.docx', '.doc']:
            return EnhancedTextExtractor._extract_word(file_path)
        elif file_ext in ['.pptx', '.ppt']:
            return EnhancedTextExtractor._extract_powerpoint(file_path)
        elif file_ext in ['.txt', '.md']:
            return EnhancedTextExtractor._extract_text_file(file_path)
        else:
            return "", "unsupported", [f"Unsupported file type: {file_ext}"]
    
    @staticmethod
    def _extract_pdf(file_path: str) -> tuple[str, str, List[str]]:
        """Extract text from PDF."""
        errors = []
    ...
    @staticmethod
    def _extract_excel(file_path: str) -> tuple[str, str, List[str]]:
        """Extract text from Excel files."""
        errors = []
    ...
    @staticmethod
    def _extract_csv(file_path: str) -> tuple[str, str, List[str]]:
        ...
        try:
            ...
            return "\n".join(text_content), "csv", errors
            
        except Exception as e:
            errors.append(f"CSV extraction error: {e}")
        
        return "", "none", errors
    
    @staticmethod
    def _extract_word(file_path: str) -> tuple[str, str, List[str]]:
        """Extract text from Word documents."""
        errors = []
        ...
        try:
            ...
            return "\n".join(text_content), "python-docx", errors
            
        except ImportError:
            errors.append("python-docx not available")
        except Exception as e:
            errors.append(f"Word extraction error: {e}")
    ...
    @staticmethod
    def _extract_powerpoint(file_path: str) -> tuple[str, str, List[str]]:
        ...
        try:
            ...
            return "\n".join(text_content), "python-pptx", errors
            
        except ImportError:
            errors.append("python-pptx not available")
        except Exception as e:
            errors.append(f"PowerPoint extraction error: {e}")
    ...
    @staticmethod
    def _extract_text_file(file_path: str) -> tuple[str, str, List[str]]:
        ...
        try:
            ...
            errors.append(f"Text file error: {e}")
        
        return "", "none", errors

class EnhancedDatasetProcessor:
    """Enhanced processor supporting multiple file types including spreadsheets."""
    
    def __init__(self, max_workers: int = 4):
        """Initialize processor."""
        self.max_workers = max_workers
        self.temp_base_dir = tempfile.mkdtemp(prefix="enhanced_dataset_processing_")
        self.stats = {
            "processed_zips": 0,
            "failed_zips": 0,
            "document_pairs": 0,
    ...
    def _process_single_zip(self, zip_file: str, category: str) -> DocumentPair:
        """Process a single ZIP file with enhanced logic."""
        ...
        try:
            # Extract ZIP with deep extraction for nested structures
            self._extract_zip_deep(zip_file, temp_dir)
            
            # Find all relevant files
            all_files = self._find_all_files(temp_dir)
            
            # Separate into content and metadata files
            content_files, metadata_files = self._separate_content_metadata(all_files, base_name)
            
            # Process metadata (should be PDF)
            if metadata_files:
                metadata_file = metadata_files[0]  # Take first metadata file
                pair.metadata_doc = self._process_file(metadata_file, category, "metadata", base_name)
                if pair.metadata_doc:
                    self.stats["metadata_docs"] += 1
            else:
                pair.errors.append("No metadata file found")
            
            # Process content (can be PDF, Excel, etc.)
            if content_files:
                content_file = content_files[0]  # Take first/largest content file
                pair.content_doc = self._process_file(content_file, category, "content", base_name)
                if pair.content_doc:
                    self.stats["content_docs"] += 1
                ...
                pair.errors.append("No content file found")
    ...
    def _extract_zip_deep(self, zip_file: str, extract_dir: str, max_depth: int = 3):
        """Extract ZIP with deep nested extraction."""
    ...
    def _find_all_files(self, directory: str) -> List[str]:
    ...
    def _separate_content_metadata(self, all_files: List[str], base_name: str) -> tuple[List[str], List[str]]:
        """Separate content and metadata files."""
        
        metadata_files = []
        content_files = []
        
        # First, identify metadata files (usually PDFs with -info suffix)
        for file_path in all_files:
            file_name = os.path.basename(file_path)
            name_without_ext = os.path.splitext(file_name)[0]
            
            if (name_without_ext.endswith('-info') or 'info' in name_without_ext.lower()) and file_name.endswith('.pdf'):
                metadata_files.append(file_path)
        
        # Then, identify content files (everything else that's not metadata)
        for file_path in all_files:
            if file_path not in metadata_files:
                file_ext = os.path.splitext(file_path)[1].lower()
                # Include various content file types
                if file_ext in ['.pdf', '.xlsx', '.xls', '.csv', '.docx', '.doc', '.pptx', '.ppt', '.txt']:
                    content_files.append(file_path)
        
        # Sort content files by size (largest first) and relevance
        if content_files:
            content_files_with_size = []
            for f in content_files:
                try:
                    size = os.path.getsize(f)
                    # Prefer files that match the base name
                    name_match = base_name.lower() in os.path.basename(f).lower()
                    content_files_with_size.append((f, size, name_match))
                except:
                    content_files_with_size.append((f, 0, False))
            
            # Sort by name match first, then size
            content_files_with_size.sort(key=lambda x: (x[2], x[1]), reverse=True)
            content_files = [f for f, _, _ in content_files_with_size]
    ...
    def _process_file(self, file_path: str, category: str, doc_type: str, base_name: str) -> Optional[ProcessedDocument]:
        ...
        try:
            ...
            doc = ProcessedDocument(
                ...
                category=category,
                doc_type=doc_type,
                content_format=file_ext[1:] if file_ext else 'unknown',
                text_content=text_content,
                metadata={
                    "base_name": base_name,
                    "extraction_method": extraction_method,
    ...
    def _save_results(self, pairs: List[DocumentPair], output_path: str):
        ...
        # Save main results
        results_file = os.path.join(output_path, f"enhanced_processing_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
        with open(results_file, 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2, ensure_ascii=False)
        ...
        for pair in pairs:
            if pair.content_doc and pair.content_doc.text_content:
                content_file = os.path.join(texts_dir, f"{pair.base_name}_content.txt")
                with open(content_file, 'w', encoding='utf-8') as f:
                    f.write(f"Format: {pair.content_doc.content_format}\n")
                    f.write(f"Method: {pair.content_doc.extraction_method}\n")
                    f.write("="*50 + "\n")
                    f.write(pair.content_doc.text_content)
    ...
    def _print_summary(self):
        ...
        if self.stats['by_file_type']:
            print(f"\n📋 Content File Types Processed:")
            for file_type, count in self.stats['by_file_type'].items():
...
def main():
    ...
    print("🚀 Enhanced Dataset Processor (with Spreadsheet Support)")
    print("="*60)
    ...
    input_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(os.path.dirname(input_path), "enhanced_processed_output")
    ...
    for name, module in deps.items():
        try:
            ...
            print(f"⚠️  {name} not available - install with: pip install {name.lower()}")
