...
# Fixed main.py - services/processor/src/main.py
import hydra
from omegaconf import DictConfig
import os
import sys
...
try:
    ...
    print("Run from project root: python -m services.processor.src.main")
    print("Or use the standalone processor instead")
    sys.exit(1)
...
def main(cfg: DictConfig):
    ...
    logger.info(f"Starting pipeline: {cfg.app_name} v{cfg.app_version}")
    
    # Initialize pipeline with minimal config to avoid missing classes
    pipeline_config = {
        "pdf": cfg.get("pdf", {}),
        "chunker": cfg.get("chunker", {}),
        "classifier": {"enabled": False}  # Disable to avoid missing dependencies
    }
    ...
    try:
        ...
        logger.info("Consider using standalone_dataset_processor.py instead")
        return
    
    # Process documents
    input_dir = cfg.processing.input_dir
    if not os.path.exists(input_dir):
        logger.warning(f"Creating input directory: {input_dir}")
        os.makedirs(input_dir, exist_ok=True)
        logger.info("Place documents in input directory and run again")
        return
    
    logger.info(f"Processing from: {input_dir}")
        ...
        for result in results:
            if result.success and result.document:
                try:
                    ...
                    logger.error(f"Storage error for {result.document.file_name}: {e}")
        
        # Print summary
        logger.info(f"Completed: {successful}/{len(results)} documents stored")
        
    except Exception as e:
        logger.error(f"Processing failed: {e}")
        logger.info("For simpler processing, use: python standalone_dataset_processor.py")
